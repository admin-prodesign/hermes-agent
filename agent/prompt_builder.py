...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (mandatory)\n"
            "Scan the skill index before replying. If any skill is relevant or partially relevant, "
            "MUST load it with skill_view(name) and follow it. Skills hold user conventions, "
            "tool/API details, safety rules, and proven workflows; load rather than guessing. "
            "For Hermes Agent setup/config/tools/skills/gateway/providers/models/plugins/voice, "
            "load `hermes-agent` first. If a loaded skill is wrong or stale, patch it. "
            "After difficult reusable work, offer/save a skill.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
