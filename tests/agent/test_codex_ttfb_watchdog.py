...
def test_codex_stream_recovers_from_null_output_parser_error():
    """If the Codex backend completes with output=null, openai-python raises
    TypeError while parsing the final snapshot. Recover from streamed deltas
    instead of surfacing a non-retryable local parser error."""
    ...
    class _BrokenCodexStream:
        ...
        def get_final_response(self):
            raise AssertionError("iterator should fail before final response")

    class _Responses:
        def stream(self, **_kwargs):
            return _BrokenCodexStream()

    agent = SimpleNamespace(
        _interrupt_requested=False,
        _codex_stream_last_event_ts=None,
        _touch_activity=lambda *_args, **_kwargs: None,
        _fire_stream_delta=lambda *_args, **_kwargs: None,
        _fire_reasoning_delta=lambda *_args, **_kwargs: None,
