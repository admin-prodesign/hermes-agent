...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        for chunk in self.truncate_message(self.format_message(content), MAX_POST_LENGTH):
            posted = await self._post_message(chat_id, chunk, reply_to, metadata)
            result = _post_result(posted, "Failed to create post")
            if result.success:
                self._enqueue_pd_one_quality_event(chat_id, {"message": chunk}, posted, metadata if isinstance(metadata, dict) else None)
    ...
    def _apply_channel_gating(self, channel_id: str, message_text: str) -> Optional[str]:
        ...
        ignored_channels = _channel_id_set(self._extra_or_env("ignored_channels", "MATTERMOST_IGNORED_CHANNELS"))
...
_YAML_BRIDGE = (  # (yaml key, env var, yaml value → env string); allowed_channels is a whitelist
    ...
    ("ignored_channels", "MATTERMOST_IGNORED_CHANNELS", _csv),
    ("auto_translate_mentioned_channel_messages", "MATTERMOST_AUTO_TRANSLATE_MENTIONED_CHANNEL_MESSAGES", lambda v: str(v).lower()),
...
def _apply_yaml_config(yaml_cfg: dict, mattermost_cfg: dict) -> dict | None:
    ...
    for key, env, to_env in _YAML_BRIDGE:
        ...
        if value is None and not (key == "require_mention" and key in mattermost_cfg):
            continue
        seeded[key] = value
        if not skip_env_bridge and env and not os.getenv(env):
            os.environ[env] = to_env(value)
    # Dict extras stay in PlatformConfig.extra only (no env serialization).
    for extra_key in (
